...
| `wrapReplace(list, start, end, value)` | `(List, Integer, Integer, List \| Tuple) -> Nil` | the write counterpart to `wrapSlice` — replaces the `wrapSlice(list, start, end)` span with `value`'s elements **in place**, with no requirement that `value` be the same length: a shorter `value` shrinks `list`, a longer one grows it, an empty `value` deletes the span. The span's positions are removed and `value`'s elements are spliced in as one block at the span's *first* index in read order, everything else keeping its original relative order — `wrapReplace(xs, 3, 5, wrapSlice(xs, 5, 3))` reverses positions `3..5` in place (writing back a backward, already-reversed read of the same span), and an ordinary in-range forward destination reduces to the same splice Python's `list[i:j] = value` does. `list` must be a real List (mutated like `push`/`setAt` — Tuple/String are immutable, nothing to resize); `value` can be a List or Tuple. An empty `list` is still a runtime error |
