...
| `wrap(collection, i)` | `(List \| Tuple \| String, Integer) -> Any` | circular/modular indexing: `collection[i]` reduced modulo `collection`'s length, so an out-of-range `i` (past the end, or negative) loops back around instead of erroring the way a plain index or `at` does. `wrap([10,20,30], 3)` is `10`; `wrap([10,20,30], -1)` is `30` — unlike a plain index (§6, not negative-wrappable), `wrap` *is* the negative-wraps-from-the-end tool. An empty `collection` is still a runtime error (no valid index to wrap onto) |
| `wrapSlice(collection, start, end)` | `(List \| Tuple \| String, Integer, Integer) -> List \| Tuple \| String` | the circular counterpart to `collection[start..end]` — always inclusive of both ends (no `.<`-style exclusive variant). Neither bound has to stay in range: each resolved position wraps modulo `collection`'s length, so a span can run past the end and keep going from the start (`wrapSlice([0,1,2,3], 3, 6)` is `[3, 0, 1, 2]`), or even lap the collection more than once. `start <= end` walks forward, `start > end` walks backward, same direction rule §6's plain slicing already uses — applied to the raw (not end-relative) `start`/`end` values, so `wrapSlice([0,1,2,3], -1, 2)` is also `[3, 0, 1, 2]` (walking forward from just before zero) while `wrapSlice([0,1,2,3], 1, -2)` walks backward through `1, 0, -1, -2` (`[1, 0, 3, 2]`). An empty `collection` is still a runtime error |
| `wrapReplace(list, start, end, value)` | `(List, Integer, Integer, List \| Tuple) -> Nil` | the write counterpart to `wrapSlice` — replaces the `wrapSlice(list, start, end)` span with `value`'s elements **in place**. When `value` is the *same length* as the span, this is a pure position-wise write-back — `list[indices[k]] = value[k]` for each of the span's positions, in read order, leaving every other position untouched, even when the span itself wraps (this is what makes `wrapReplace(xs, 3, 5, wrapSlice(xs, 5, 3))` reverse positions `3..5` in place, including across the wrap point — a knot-hash/AoC-2017-day-10-style "pin a wrapping span, reverse it, repeat" algorithm). When `value` is a *different length*, there's no single position each new element belongs to, so the span's positions are removed and `value`'s elements are spliced in instead, as one block at the span's *first* index in read order, everything else keeping its original relative order — a shorter `value` shrinks `list`, a longer one grows it, an empty `value` deletes the span, and an ordinary in-range forward destination reduces to the same splice Python's `list[i:j] = value` does. `list` must be a real List (mutated like `push`/`setAt` — Tuple/String are immutable, nothing to resize); `value` can be a List or Tuple. An empty `list` is still a runtime error |
...
| `findInts(s)` | `(String) -> List` | every integer embedded anywhere in `s`, in order — "extract the numbers from this line," the messier real-world counterpart to `ints(split(line))`'s "the whole line is nothing but digit-strings." A `-` counts as a sign only when it directly precedes a digit *and* isn't itself preceded by a digit — `findInts("1-3 a: abcde")` is `[1, 3]` (a range, not `-3`), while `findInts("y=-10..-5")` is `[-10, -5]` |
...
| `filter(iterable, fn)` | `(List \| Tuple, Function) -> List` | keeps only the elements of a List or Tuple where `fn(element)` is truthy (§6's truthiness rule — the same one `order`/`hold`/`with`/`or` use), collecting survivors into a new List in their original order |
| `reduce(iterable, fn, init)` | `(List \| Tuple, Function, Any) -> Any` | folds a List or Tuple down to a single value by calling `fn(accumulator, element)` once per element in order, starting from `init`, and returning whatever the last call produced. `init` is required rather than optional, so an empty `iterable` has an unambiguous, non-error result — `reduce([], fn, 0)` is just `0` |
| `any(iterable, fn)` | `(List \| Tuple \| Set, Function) -> Boolean` | `stuffed` the moment `fn(element)` is truthy for some element (short-circuiting, like `or`); `thin` if none are, or `iterable` is empty |
| `all(iterable, fn)` | `(List \| Tuple \| Set, Function) -> Boolean` | `thin` the moment `fn(element)` is falsy for some element (short-circuiting); `stuffed` if every element passes, or `iterable` is empty (vacuous truth) |
...
| `pop(list)` / `pop(list, i)` / `pop(map, key)` / `pop(set, item)` | `(List) -> Any` / `(List, Integer) -> Any` / `(Map, Any) -> Any` / `(Set, Any) -> Any` | removes and returns a value in place — the return-value counterpart to `push`/`sprinkle`/`scrape`, none of which hand back what they touched. `pop(list)` removes and returns the last element (a runtime error on an empty `list`). `pop(list, i)` removes and returns the element at `i`; like plain `list[i]` (§6), `i` doesn't wrap negative — out of range is a runtime error either way. `pop(map, key)` removes and returns the value at `key`; a present-but-wrong-type `key` errors the same way `map[key]` does on read, but a valid, merely-absent `key` returns `nobox`, matching every other Map read. `pop(set, item)` removes and returns `item` if it's a member, or `nobox` if not — `item`'s type is never checked, matching `scrape`'s permissiveness rather than Map's stricter key rule |
...
| `sum(x)` | `(List \| Tuple \| Set) -> Integer \| Float` | total of `x`'s elements (Integers/Floats only) — widens to Float the moment any element is a Float, the same "widen only when you have to" rule `+` itself follows. `sum([])` is `0`, an Integer, not an error |
| `pizzasort(list)` | `(List \| Tuple) -> List` | `list`'s elements sorted into natural order — same ordering rule as `<`/`min`/`max` (numbers freely mixed, or Strings, never both). Returns a new List; `list` itself is unchanged, even if it was already a List |
| `sortBy(list, fn)` | `(List \| Tuple, Function) -> List` | `pizzasort`'s key-function counterpart: `list`'s elements reordered by comparing `fn(element)` — same numbers-together-or-Strings-together rule — instead of the elements themselves. Stable: elements whose keys are equal keep their original relative order |
| `reverse(x)` | `(List \| Tuple \| String) -> List \| Tuple \| String` | `x`'s elements (or, for a String, its runes) in reverse order, as a new value — `x` itself is never mutated. No Set case; a Set has no order to reverse (§2.2) |
...
| `zip(a, b)` | `(List \| Tuple, List \| Tuple) -> List` | pairs `a` and `b` element-wise into a List of `(a[i], b[i])` Tuples, truncating to the shorter of the two rather than erroring on a length mismatch (Python's `zip()` convention). Every paired element must be Hashable, same requirement as `enumerate` |
...
| `manhattan(a, b)` | `(Tuple, Tuple) -> Integer` | taxicab/Manhattan distance between two `(row, col)` Tuples — `abs(a.row - b.row) + abs(a.col - b.col)`. Pure coordinate arithmetic, same no-bounds-checking posture as `neighbors4`/`neighbors8` |
| `heapify(list)` | `(List) -> Nil` | reorders `list`'s elements in place into min-heap order, in O(n). There's no separate priority-queue type — any List becomes one the moment `heapify`/`heapPush`/`heapPop`/`heapPeek` are called on it, the same "mutate the List you're given" posture `push`/`sprinkle`/`pop` already take |
| `heapPush(list, item)` | `(List, Any) -> Nil` | appends `item` and sifts it up into its correct min-heap position, in place. Assumes `list` is already a valid heap (built by `heapify`, or empty) |
| `heapPop(list)` | `(List) -> Any` | removes and returns `list`'s smallest element, in place, restoring the heap invariant afterward. Errors on an empty heap. Elements are compared the same way `<`/`min`/`max`/`pizzasort` compare (numbers freely mixed, or Strings), plus one addition: two Tuples compare lexicographically, so `heapPush(pq, (dist, node))` — the standard Dijkstra/A* idiom, priority paired with payload — orders by `dist` first and only falls back to `node` on a tie |
| `heapPeek(list)` | `(List) -> Any` | `list`'s smallest element without removing it. Errors on an empty heap, same as `heapPop` |
...
| `band(a, b)` | `(Integer, Integer) -> Integer` | bitwise AND, Integer-only like `idiv`/`gcd`/`lcm`. Named with a `b` prefix for consistency across the whole family — `or` is the one name that's actually reserved (§4's own logical OR keyword; `and`/`not` are free, since cRust's logical AND/NOT are the `with`/`hold` keywords instead) |
| `bor(a, b)` | `(Integer, Integer) -> Integer` | bitwise OR |
| `bxor(a, b)` | `(Integer, Integer) -> Integer` | bitwise XOR — the most commonly reached-for of this family in AoC-style puzzles (checksums, parity, hash mixing, ...) |
| `bnot(x)` | `(Integer) -> Integer` | bitwise NOT (one's complement): every bit flipped, over the full 64-bit width Integer already uses. A numeric transform, not `hold`/`!`'s logical NOT on a Boolean — `bnot(0)` is `-1` |
| `shl(x, n)` | `(Integer, Integer) -> Integer` | left shift: `x`'s bits moved toward the high end by `n` places, zero-filling from the low end. A negative `n` is a runtime error, not a process-crashing panic |
| `shr(x, n)` | `(Integer, Integer) -> Integer` | right shift: `x`'s bits moved toward the low end by `n` places. Arithmetic, not logical — the sign bit fills in from the high end (`shr(-8, 1)` is `-4`), since cRust has no unsigned Integer type to make a logical shift meaningful against. A negative `n` is a runtime error, same as `shl` |
| `rebox(element, fromBase, toBase)` | `(String, Integer, Integer) -> String` | base conversion — leans on the same "pizza box" metaphor `nobox` already established (§4): converting a number's base is repacking the exact same value into differently-sized boxes, nothing about the value itself changes. `element` is always a String (the digit representation being converted, in `fromBase`) and the result is always a String too, even for `toBase == 10` — never an Integer either way, so there's one predictable rule instead of a special case for base 10. `fromBase`/`toBase` must be `2..36` (2-9 use digits, 10-35 add letters, one per value past 9); input letters are accepted in either case, output is always lowercase. `rebox("255", 10, 2)` is `"11111111"`; `rebox("ff", 16, 10)` is `"255"` |
...
| `keys(m)` / `values(m)` | `(Map) -> List` | `m`'s keys, or `m`'s values, each as a new List. Deliberately in the *same* order as each other (sorted by each key's text form) rather than whatever order Map's own internal storage happens to visit — Go's map iteration is randomized per call, so without an explicit shared order, `keys(m)[i]` and `values(m)[i]` from two separate calls could silently end up as two different entries. `knead k in m` (§8) already iterates `m`'s keys directly; reach for `keys`/`values` specifically when you want both as real Lists, e.g. to zip together by index |
