...
var builtinDocs = map[string]string{
	...
	"grid":       "`grid(s) -> List` — parses s into a row-major grid: a List of rows, each row a List of one-character Strings. Pair with at/setAt/neighbors4/neighbors8.",
	"at":         "`at(g, pos) -> Any` — bounds-checked read from grid g at (row, col) Tuple pos; reads as nobox if out of range instead of erroring, unlike plain g[row][col].",
	"setAt":      "`setAt(g, pos, value)` — bounds-checked in-place write into grid g at pos. Unlike at, out-of-range is a runtime error; the target row must be a List (not a Tuple).",
