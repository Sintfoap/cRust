...
var builtinDocs = map[string]string{
	...
	"wrapReplace": "`wrapReplace(list, start, end, value) -> Nil` — write counterpart to wrapSlice: replaces the wrapSlice(list, start, end) span with value's elements in place. value's length can differ from the span's (shrinks/grows list, empty deletes it). wrapReplace(xs, 3, 5, wrapSlice(xs, 5, 3)) reverses positions 3..5 in place. list must be a List; value a List or Tuple.",
