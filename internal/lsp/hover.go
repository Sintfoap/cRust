...
var builtinDocs = map[string]string{
	...
	"wrapReplace": "`wrapReplace(list, start, end, value) -> Nil` — write counterpart to wrapSlice: replaces the wrapSlice(list, start, end) span with value's elements in place. Same length as the span: position-wise write-back (list[indices[k]] = value[k]), even across a wrap -- wrapReplace(xs, 3, 5, wrapSlice(xs, 5, 3)) reverses positions 3..5 in place. Different length: spliced in as a block instead (shrinks/grows list, empty deletes it). list must be a List; value a List or Tuple.",
	...
	"band":        "`band(a, b) -> Integer` — bitwise AND.",
	"bor":         "`bor(a, b) -> Integer` — bitwise OR.",
	"bxor":        "`bxor(a, b) -> Integer` — bitwise XOR.",
