...
// objectsEqual is SPEC.md §6's `==`/`!=` rule: value equality
// (Lists/Maps/Sets compare contents, not identity), cross-category
// comparison is always false rather than an error. Int/Float are
// treated as one "number" category — SPEC.md's ordering rule groups
// them as "number-vs-number" explicitly, and equality follows the same
// split for consistency (1 == 1.0 is stuffed).
...
func mapsEqual(a, b *object.Map) bool {
	if len(a.Pairs) != len(b.Pairs) {
		return false
	}
	for _, pair := range a.Pairs {
		bv, ok := b.Get(pair.Key)
		if !ok || !objectsEqual(pair.Value, bv) {
			return false
		}
	}
	return true
...
func setsEqual(a, b *object.Set) bool {
	if a.Len() != b.Len() {
		return false
	}
