...
// dayFileTemplateFmt is filled in with the day number six times (the
// file header, the `crust develop` line, and each of the two `crust
// run --store=` example lines needing it twice) via fmt.Sprintf.
// Deliberately the same store_part1/store_part2 shape
// examples/dayNN_template.crust uses — one shape taught in one place —
// rather than inventing a second starter layout just because this
// one gets its placeholder filled in automatically instead of by
// hand.
const dayFileTemplateFmt = `// AoC 2026 Day %02d starter.
//
...
// parseNewArgs pulls a day number and the optional --force flag out of
// args, in any order — the same shape parseFetchArgs/parseDoneArgs
// already give their own commands, minus --year/--out since a new
// day's file always follows the one fixed dayNN.crust naming.
func parseNewArgs(args []string) (day int, force bool, err error) {
	dayStr := ""
	for _, a := range args {
		switch {
		case a == "--force":
	...
	if dayStr == "" {
		return 0, false, fmt.Errorf("missing <day>")
	}
	day, convErr := strconv.Atoi(dayStr)
	if convErr != nil || day < 1 || day > 25 {
		return 0, false, fmt.Errorf("invalid day %q (want 1-25)", dayStr)
	}
	return day, force, nil
}
...
func runNew(day int, force bool, stdout, stderr io.Writer) int {
	...
	content := fmt.Sprintf(dayFileTemplateFmt, day, day, day, day, day, day)
	if err := os.WriteFile(path, []byte(content), 0o644); err != nil {
		fmt.Fprintf(stderr, "crust new: writing %s: %s\n", path, err)
		return 1
	}

	fmt.Fprintf(stdout, "Created %s. `crust fetch %d` to grab its input, `crust develop %s` to start writing.\n", path, day, path)
	return 0
}
