...
func TestRun(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "develop missing file",
			args:       []string{"develop"},
			wantCode:   2,
			wantStderr: "develop needs a program file",
		},
		{
			name:       "develop missing on disk",
			args:       []string{"develop", "/no/such/file.crust"},
			wantCode:   1,
			wantStderr: "no such file",
		},
		{
			name:       "develop with a real file",
			args:       []string{"develop", "../../examples/hello.crust"},
			wantCode:   0,
			wantStdout: "step",
		},
		{
